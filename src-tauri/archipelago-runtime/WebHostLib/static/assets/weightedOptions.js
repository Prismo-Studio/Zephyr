...
window.addEventListener('load', () => {
	...
	document.addEventListener('click', (evt) => {
		...
		if (evt.target.classList.contains('range-option-delete')) {
			const targetRow = document.querySelector(
				`tr[data-row="${evt.target.getAttribute('data-target')}"]`
			);
			setDeletedOption(
	...
	document.getElementById('weighted-options-form').addEventListener('submit', (evt) => {
		...
		document.querySelectorAll('input[name]').forEach((input) => {
			...
			// Ensure keys exist
			if (!weightedOptions[optionName]) {
				weightedOptions[optionName] = {};
			}
			...
			if (subOption) {
				return (weightedOptions[optionName][subOption] = determineValue(input));
			}
			if (optionName) {
				return (weightedOptions[optionName] = determineValue(input));
			}
		});
...
const unsetDeletedOption = (optionName, value) => {
	if (!deletedOptions.hasOwnProperty(optionName)) {
		return;
	}
	if (deletedOptions[optionName].includes(`${optionName}-${value}`)) {
		deletedOptions[optionName].splice(
			deletedOptions[optionName].indexOf(`${optionName}-${value}`),
			1
		);
	}
