...
window.addEventListener('load', () => {
	const tables = $('.table').DataTable({
		...
		footerCallback: function (tfoot, data, start, end, display) {
			if (tfoot) {
				const activityData = this.api()
					.column('lastActivity:name')
					.data()
					.toArray()
					.filter((x) => !isNaN(x));
			...
			},
			{
				...
				render: function (data, type, row) {
					if (type === 'sort' || type === 'type') {
						if (data === 'None') return Number.MAX_VALUE;
	...
	window.addEventListener('keydown', (event) => {
		...
		if (!event.ctrlKey && !event.altKey && !event.shiftKey && event.key === 'Escape') {
			...
			searchBox.blur();
			if (!document.getElementById('tables-container')) window.scroll(0, 0);
			event.preventDefault();
		}
	});
	const target_second =
		parseInt(document.getElementById('tracker-wrapper').getAttribute('data-second')) + 3;
