<script lang="ts">
	...
	function handleButton(buttonIndex: number) {
		const active = document.activeElement as HTMLElement;
		const btnNames = ['A','B','X','Y','LB','RB','LT','RT','SEL','START','L3','R3','UP','DOWN','LEFT','RIGHT'];
		...
		switch (buttonIndex) {
			...
			case BUTTON.SELECT:
				// Toggle filters
				{
					const filterBtn = document.querySelector('.z-filter-toggle-btn, [class*="filter"]') as HTMLElement;
...
{#if connected}
	<div class="z-gamepad-indicator" title="Controller connected">
		{#if debugInfo}<span class="z-gamepad-debug">{debugInfo}</span>{/if}
		<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
			<path d="M7.97 16L5 19c-.69.69-1 .05-1-.5V5c0-.55.45-1 1-1h14c.55 0 1 .45 1 1v14.5c0 .55-.31 1.19-1 .5l-2.97-3H7.97z" opacity="0" />
			<path
				d="M17.5 6h-11A2.5 2.5 0 0 0 4 8.5v4A2.5 2.5 0 0 0 6.5 15h1.59l-1.5 2.25a1.25 1.25 0 1 0 2.08 1.39L10.92 15h2.16l2.25 3.64a1.25 1.25 0 1 0 2.08-1.39L15.91 15h1.59a2.5 2.5 0 0 0 2.5-2.5v-4A2.5 2.5 0 0 0 17.5 6zM9 12H8v1a.75.75 0 0 1-1.5 0v-1H5.5a.75.75 0 0 1 0-1.5h1V9.5a.75.75 0 0 1 1.5 0v1H9a.75.75 0 0 1 0 1.5zm5.25 1a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm2.5-2.5a1 1 0 1 1 0-2 1 1 0 0 1 0 2z"
			/>
		</svg>
	</div>
{/if}
